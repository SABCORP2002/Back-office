  ...
  return (
    <Screen>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)]">
        ...
        <Card padded={false}>
          <div className="p-4 pb-2 sm:p-5 sm:pb-3"><CardTitle action={<Badge tone="info">Revue manuelle</Badge>}>Candidatures Ambassadeur</CardTitle><p className="-mt-3 text-xs leading-5 text-onSurfaceVariant">Les liens des réseaux sociaux et la décision sont conservés pour l’équipe autorisée. L’approbation débloque le choix du code, pas ses conditions commerciales.</p></div>
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border text-left text-xs text-onSurfaceVariant"><th className="px-4 py-3 font-medium">Candidat</th><th className="px-4 py-3 font-medium">Réseaux sociaux</th><th className="px-4 py-3 font-medium">Code</th><th className="px-4 py-3 font-medium">Statut</th><th className="px-4 py-3 font-medium"></th></tr></thead><tbody>{ambassadorApplications.slice(0, 12).map((row) => <tr key={row.id} className="border-b border-border/60 align-top"><td className="px-4 py-3"><div className="font-medium">{row.displayName}</div><div className="mt-1 text-xs text-onSurfaceVariant">{identity(row.user)} · {row.user.country}</div><div className="mt-1 text-xs text-onSurfaceVariant">KYC : {row.user.kycStatus}</div></td><td className="px-4 py-3 text-xs"><div className="flex max-w-[250px] flex-wrap gap-1.5">{([['TikTok', row.tiktokUrl], ['Facebook', row.facebookUrl], ['YouTube', row.youtubeUrl], ['Instagram', row.instagramUrl], ['Telegram', row.telegramUrl], ['WhatsApp', row.whatsappUrl]] as Array<[string, string | null]>).filter(([, link]) => Boolean(link)).map(([platform, link]) => <a key={platform} href={link!} target="_blank" rel="noreferrer" className="rounded border border-border bg-surface-higher px-2 py-1 text-primary hover:bg-primary/10">{platform}</a>)}</div></td><td className="whitespace-nowrap px-4 py-3 font-mono text-primary">{row.promoCampaign?.code ?? 'Après approbation'}<div className="mt-1 font-sans text-xs text-onSurfaceVariant">{row.promoCampaign ? `${row.promoCampaign.redemptionsReserved} utilisation(s)` : ''}</div></td><td className="px-4 py-3"><Badge tone={row.status === 'APPROVED' ? 'success' : row.status === 'PENDING' ? 'warning' : 'error'}>{labelAmbassador(row.status)}</Badge>{row.rejectionReason && <div className="mt-1 max-w-[180px] text-xs text-error">{row.rejectionReason}</div>}</td><td className="whitespace-nowrap px-4 py-3 text-right">{row.status === 'PENDING' && canReviewAmbassadors && <div className="flex justify-end gap-2"><Button className="min-h-8 px-2 py-1 text-xs" onClick={() => approveAmbassador(row)}>Approuver</Button><Button variant="danger" className="min-h-8 px-2 py-1 text-xs" onClick={() => rejectAmbassador(row)}>Refuser</Button></div>}</td></tr>)}{!loading && ambassadorApplications.length === 0 && <tr><td colSpan={5} className="px-4 py-7 text-center text-onSurfaceVariant"><UserRoundCheck size={18} className="mr-2 inline" />Aucune candidature Ambassadeur.</td></tr>}</tbody></table></div>
        </Card>
      </div>
